...
import conjunctionData from "./other.json";
...
const Other = () => {
  ...
  const renderRules = (rules) => (
    <ul className="list-disc list-inside mt-1 space-y-1 ml-1 border rounded-md border-pink-600 mb-2">
      {rules.map((rule, idx) => (
        <li key={idx} className="normal-case ml-2">
          {rule}
        </li>
    ...
    <Container>
      <div className="max-w-4xl min-h-screen mx-auto p-4">
        ...
        <h4 className="p-3 text-xl font-semibold bg-gradient-to-r from-slate-900 via-cyan-800 to-cyan-600   text-white rounded-t-md">
          <span className="ml-4"> Nicht Konjunktion</span>
        </h4>
          ...
          <div key={index} className="">
            ...
            {expandedWords[item.conjunction] && (
              <div className="p-3 bg-gray-50 text-gray-700">
                <p className="font-medium text-lg uppercase ml-4">
                  <span className="ml-1 bg-red-600 rounded p-1 text-white font-bold">
                    {item.ruleName}
                  </span>

                  {renderRules(item.rules, item)}
                </p>

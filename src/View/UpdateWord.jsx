...
//         `/word/update/${updatedFormData.id}`,
//         dataToSend
//       );
...
//       } catch (error) {
//         console.error("Error fetching word data:", error);
//         Swal.fire("Error!", "Failed to load word data", "error");
//       }
//     };

//     const fetchInitialData = async () => {
//       try {
//         const [levelsRes, topicsRes, articlesRes, posRes] = await Promise.all([
//           api.get("/levels"),
//           api.get("/topics"),
//           api.get("/articles"),
...
//     if (["levelId", "topicId", "articleId", "partOfSpeechId"].includes(name)) {
//       setFormData((prev) => ({ ...prev, [name]: value }));
//     } else if (["value", "pluralForm"].includes(name)) {
//       setFormData((prev) => ({ ...prev, [name]: value }));
//     } else {
...
//             `/words/suggestions?query=${value}&type=${name}`
//           );
//           setSuggestions((prev) => ({ ...prev, [name]: res.data }));
//         } catch (error) {
//           console.error("Error fetching suggestions:", error);
//         }
//       } else {
...
//   //      try {
//   //        await api.put(`/word/update/${id}`, { [field]: updatedItems });
...
//       const response = await api.put(
//         // `/word/update/${updatedFormData.id}`,
//         `/update-word/${updatedFormData.id}`,
//         dataToSend
//       );
//       console.log("Response:", response);
//     } catch (error) {
...
//           partOfSpeechId: formData.partOfSpeechId
//             ? Number(formData.partOfSpeechId)
//             : null,
//           pluralForm: formData.pluralForm || null,
//         };

//         // const response = await api.put(`/word/update/${id}`, payload);
//         const response = await api.put(`/update-word/${id}`, payload);

//         setFormData((prev) => ({
//           ...prev,
//           ...response.data.data,
//           synonyms: response.data.data.synonyms || prev.synonyms,
//           antonyms: response.data.data.antonyms || prev.antonyms,
//           similarWords: response.data.data.similarWords || prev.similarWords,
//         }));
...
const UpdateWord = () => {
  ...
  const updateWordData = async (updatedFormData) => {
    try {
      // Send the updated data to the backend
      const response = await api.put(`/update-word/${updatedFormData.id}`);
      console.log("Response:", response);
    } catch (error) {
      console.error("Error updating word:", error.response || error);
    }
